...
class Employee(BaseModel):
    ...
    photo: Optional[str] = Field(
        default="<<your big base64 default here>>",
...
fastapi_app = FastAPI(
    title="HR REST API",
    version="0.0.1",
    description="FastAPI implementation of the HR service"
)
...
@fastapi_app.get(
    "/hr/api/v1/employees/{identity}",
    response_model=Employee,
    responses={404: {"model": StatusMessage}},
    tags=["employees"],
)
async def get_employee_by_identity(identity: str):
    """
...
@fastapi_app.post(
    ...
    responses={409: {"model": StatusMessage}},
    tags=["employees"],
)
async def hire_employee(employee: Employee):
    """
    Hire a new employee.
    """
    emp_dict = employee.model_dump()
    emp_dict["_id"] = employee.identity

    if employees_collection.find_one({"_id": emp_dict["_id"]}):
        raise HTTPException(
    ...
    return StatusMessage(status="ok", message="Employee hired")
...
async def update_employee(identity: str, update: EmployeeUpdate):
    """
    ...
    return StatusMessage(status="ok", message="Employee updated")
...
async def fire_employee(identity: str):
    """
...
# uvicorn hr-api-fastapi:app --reload --port 7001
